...
{
    public class TicketsPageRequestHandler : BaseHandler, IRequestHandler<TicketsPageRequest, PagedDataDto<TicketGridItemDto>>
    {
        private readonly GuestInvitesService ticketsService;
        private readonly IdentityService identityService;

        public TicketsPageRequestHandler(
            ILogger<TicketsPageRequestHandler> logger,
            IMapper mapper,
        ...
        public async Task<PagedDataDto<TicketGridItemDto>> Handle(TicketsPageRequest request, CancellationToken cancellationToken)
        {
            ...
            var sorting = request.Sorting.Length == 0
                ...
                : request.Sorting;
            var data = await ticketsService.Get(sorting, filter, start, request.PageSize, cancellationToken);
            var userIds = data
            ...
            var items = data
                .Select(r => Mapper.Map<Ticket, TicketGridItemDto>(r, o =>
                {
                    o.AfterMap((s, d) =>
                    {
                        if (s.IssuerId != null)
                ...
                .ToArray();
            var result = new PagedDataDto<TicketGridItemDto>
            {
                Items = items,
                TotalCount = await ticketsService.Count(filter, cancellationToken)
            };
            return result;
        }
    }
