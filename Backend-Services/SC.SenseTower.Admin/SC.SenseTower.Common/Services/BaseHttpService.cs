...
namespace SC.SenseTower.Common.Services
{
    public class BaseHttpService : BaseService, IBaseHttpService
    {
        ...
        public async Task<T?> PostAsJson<T>(string? token, string url, object? data, CancellationToken cancellationToken)
        {
            httpClient.SetBearerToken(token);
            if (httpClient.DefaultRequestHeaders.Accept.All(r => r.MediaType != "application/json"))
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            using var st = new MemoryStream();
            var json = JsonSerializer.Serialize(data, new JsonSerializerOptions { PropertyNamingPolicy = JsonNamingPolicy.CamelCase, DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull });
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await httpClient.PostAsync(url, content, cancellationToken);
            return await GetResponseContent<T>(response);
        }

        public async Task<T?> PutAsJson<T>(string? token, string url, object? data, CancellationToken cancellationToken)
        {
            httpClient.SetBearerToken(token);
            if (httpClient.DefaultRequestHeaders.Accept.All(r => r.MediaType != "application/json"))
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
        ...
        public async Task<T?> Delete<T>(string? accessToken, string? refreshToken, string url, CancellationToken cancellationToken)
        {
